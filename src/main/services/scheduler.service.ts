...
export class SchedulerService {
  ...
  constructor(intervalMinutes: number, thresholdMbps: number, connectionType = 'auto') {
  ...
  private startTickTimer(): void {
    ...
    this.tickTimer = setInterval(() => {
      const nextInSeconds = Math.max(0, Math.round((this.nextTestAt - Date.now()) / 1000))
  ...
  private async executeTest(): Promise<void> {
    ...
    try {
      const [result, netInfo] = await Promise.all([runSpeedTest(), getNetworkInfo()])

      const db = getDb()
      const isSlow = result.download < this.thresholdMbps ? 1 : 0

      const resolvedType =
        this.connectionTypeOverride !== 'auto' ? this.connectionTypeOverride : netInfo.connectionType

      // ISP pode ser substituído pelo valor configurado pelo usuário
      const userIsp = (
        db.prepare("SELECT value FROM settings WHERE key = 'isp_name'").get() as
          | { value: string }
          | undefined
      )?.value
      const ispName = userIsp && userIsp.trim() !== '' ? userIsp : netInfo.ispName
