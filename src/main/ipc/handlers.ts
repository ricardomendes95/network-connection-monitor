...
export function registerHandlers(scheduler: SchedulerService): void {
  ...
  ipcMain.handle(IPC_CHANNELS.EXPORT_EVIDENCE, async (event, days: number = 30, opts?: { contracted_mbps?: number; connection_type?: string }) => {
    const db = getDb()
    const win = BrowserWindow.fromWebContents(event.sender)
    ...
    let wifiThreshold = 0
    if (contracted > 0) {
      wiredThreshold = contracted * 0.40
      wifiThreshold = contracted * 0.30
    }
    if (wiredThreshold <= 0) {
      const row = db.prepare("SELECT value FROM settings WHERE key = 'slow_threshold_mbps'").get() as { value: string } | undefined
    ...
    const ispRow = db.prepare("SELECT value FROM settings WHERE key = 'isp_name'").get() as { value: string } | undefined
    ...
    const stats = db.prepare(`
      ...
      FROM speed_results
      WHERE tested_at >= datetime('now', '-' || ? || ' days')
    `).get(wifiThreshold, wiredThreshold, wifiThreshold, wiredThreshold, days) as {
      total: number; violations: number; avg_download: number
      avg_ping: number; avg_packet_loss: number | null
      total_days: number; violation_days: number
    }

    const worstHour = db.prepare(`
    ...
    `).get(days, wifiThreshold, wiredThreshold) as { hour: string; cnt: number } | undefined
    ...
    const violPct = stats.total > 0 ? ((stats.violations / stats.total) * 100).toFixed(1) : '0'
    const avgPct = contracted > 0 ? ` (${((stats.avg_download / contracted) * 100).toFixed(0)}% do plano)` : ''
    const threshLabel = contracted > 0
    ...
    const worstHourLabel = worstHour
      ...
      : 'N/A'
    const packetLossLabel = stats.avg_packet_loss != null
    ...
    const lines = [
      ...
      row('Plano contratado:', contracted > 0 ? `${contracted} Mbps` : 'Não configurado'),
      row('Mínimo exigido ANATEL:', threshLabel),
      row('Período analisado:', `${dateRange.first_day ? fmtDate(dateRange.first_day) : '—'} a ${dateRange.last_day ? fmtDate(dateRange.last_day) : '—'} (${days} dias)`),
      '',
      row('RESUMO ESTATÍSTICO'),
      row('Total de testes realizados:', stats.total),
      row('Testes abaixo do mínimo ANATEL:', `${stats.violations} (${violPct}%)`),
      row('Dias com ao menos 1 violação:', `${stats.violation_days} de ${stats.total_days} dias`),
      row('Média de download no período:', `${stats.avg_download.toFixed(1)} Mbps${avgPct}`),
      ...
      row('Data/Hora', 'Download (Mbps)', '% do Plano', 'Upload (Mbps)', 'Ping (ms)', 'Jitter (ms)', 'Perda de Pacotes (%)', 'Tipo de Conexão', 'Rede', 'ISP', 'Servidor', 'Link de Verificação Ookla'),
      ...violations.map((r) => {
        const pct = contracted > 0 ? `${((r.download / contracted) * 100).toFixed(0)}%` : 'N/A'
        const connLabel = r.connection_type === 'wifi' ? 'WiFi' : r.connection_type === 'wired' ? 'Cabo' : ''
        return row(
  ...
  ipcMain.handle(IPC_CHANNELS.EXPORT_EVIDENCE_PDF, async (event, days: number = 30, opts?: { contracted_mbps?: number; connection_type?: string }) => {
    const db = getDb()
    const win = BrowserWindow.fromWebContents(event.sender)
    ...
    if (wiredThreshold <= 0) {
      const tRow = db.prepare("SELECT value FROM settings WHERE key = 'slow_threshold_mbps'").get() as { value: string } | undefined
    ...
    const stats = db.prepare(`
      ...
      WHERE tested_at >= datetime('now', '-' || ? || ' days')
    `).get(wifiThreshold, wiredThreshold, wifiThreshold, wiredThreshold, days) as {
      total: number; violations: number; avg_download: number; avg_upload: number
      avg_ping: number; avg_packet_loss: number | null
      total_days: number; violation_days: number
    }
    ...
    interface DailyRow {
      day: string; connection_type: string; network_name: string | null
      total: number; violations: number
      min_download: number; avg_download: number; avg_upload: number
    }
    ...
    interface ViolRow {
      tested_at: string; download: number; upload: number; ping: number
      jitter: number | null; packet_loss: number | null
      connection_type: string | null; network_name: string | null
      server_name: string | null; server_host: string | null; result_url: string | null
    }
    const violations = db.prepare(`
      SELECT tested_at, download, upload, ping, jitter, packet_loss,
    ...
    const threshLabel = contracted > 0
      ...
      : `${wiredThreshold.toFixed(0)} Mbps (threshold configurado)`
    const worstHourLabel = worstHour ? `${worstHour.hour}:00 – ${worstHour.hour}:59 (${worstHour.cnt} ocorrências)` : 'N/A'
    ...
    const dailyRowsHtml = dailyStats.map((d) => {
      const rate = d.total > 0 ? ((d.violations / d.total) * 100).toFixed(0) : '0'
      ...
      const netLabel = isWifi && d.network_name ? `<br><span style="font-size:9px;color:#9ca3af">${h(d.network_name)}</span>` : ''
      return `<tr>
    ...
    const violRowsHtml = violations.map((r) => {
      const pct = contracted > 0 ? `${((r.download / contracted) * 100).toFixed(0)}%` : '—'
      const isWifi = r.connection_type === 'wifi'
      const connLabel = isWifi ? '📶 WiFi' : r.connection_type === 'wired' ? '🔌 Cabo' : '—'
      const netLabel = isWifi && r.network_name ? `<br><span style="font-size:8.5px;color:#9ca3af">${h(r.network_name)}</span>` : ''
    ...
    const packetLossCell = stats.avg_packet_loss != null
...
<div class="footer">
  <span>Network Connection Monitor · ${h(now.toLocaleString('pt-BR'))}</span>
